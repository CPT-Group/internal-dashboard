...
- **Dev Corner Two (RIGHT TV)** — **Company-facing**. Visible to non-dev employees. 7-slide carousel with **per-slide dwell** (`SLIDE_DURATIONS_MS` in `DevCornerTwoDashboard.tsx`): ~30s for slides 1–4 and 6; **2 min** for slide 5 (Dev Load Matrix) and slide 7 (GitHub feed):
  ...
  - Slide 7: **GitHub — recent deliveries** (`GithubActivitySlide`): same **`GET /api/webhooks/github`** cache as `/tv/github-activity` (not duplicate Jira analytics). **`DEV_CORNER_TWO_FIXED_SLIDE_INDEX`** in `DevCornerTwoDashboard.tsx` can pin the carousel on this slide (`6` in 0-based); set to `null` for normal rotation.
  ...
  - Components: `InProgressCardsSlide`, `RecentlyCompletedSlide`, `RequestedTicketsSlide`, `TodayComponentVelocitySlide`, `DevLoadMatrixSlide`, `CompletedByDevSlide`, `GithubActivitySlide`.
