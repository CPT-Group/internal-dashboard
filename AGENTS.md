...
- **Dev Corner Two (RIGHT TV)** — **Company-facing**. Visible to non-dev employees. Carousel slides and dwell times are configured in **`devCornerTwoSlides.config.ts`**: each slide has **`enabled: true | false`** and **`durationMs`** (e.g. **25s** for Jira slides, **300s** for GitHub). Only enabled slides are mounted and rotated; order is fixed in that file. **`DEV_CORNER_TWO_FIXED_SLIDE_INDEX`** pins a **0-based index among enabled slides only** (e.g. `0` when GitHub is the sole enabled slide); `null` = auto-advance.
  ...
  - **GitHub — CD deploy status** (`GithubDeployStatusSlide`): **`GET /api/github/deploy-status`** (Actions API; token chain `GH_MASTER_PAT_KYLE` → `GH_ROY_PAT_MASTER_CLASSIC` → `GITHUB_TOKEN_3` → `GITHUB_TOKEN_2` → `GITHUB_DEPLOY_READ_TOKEN`).
...
- Sync Kyle/James daily hours on **In Dev** tickets via **`/cpt-sr-dev-time-sync`** (`set-in-progress-worklogs-today.mjs`); **always `--apply` immediately** — never dry-run or ask for confirmation; supports absolute hours, **`k:+1` / `j:-0.5` deltas** from today's total, and **21h max/person/day**; add **`--include-dev-review`** only when the user asks to include Dev Review; use **`adjust-worklogs-today.mjs`** only to trim/scale existing today entries proportionally by ticket.
...
- Dev Corner Two **CD deploy cards** watch swim-lane workflows only (Dev Fast / TST Build / Deploy Version / in-flight **TST Auto-Merge** on AZF, internal-tools, EF, NuGet, P2P) — **not** CI, PR auto-merge, or Dependabot; a busy Actions/PR page can look active while the TV stays idle. NuGet is **2-lane** Prerelease/Release (legacy Publish NuGet workflow deleted). Poll: **`GITHUB_DEPLOY_STATUS_POLL_INTERVAL_MS`** 25s + ~20s server cache (webhook activity feed still 60s).
...
- Dev Corner Two deploy lanes: **N/A is reserved for package/npm libs that never deploy** — never for empty/failed API data. Deployments API failure → **API ERR**. In-flight **Deploy Version** overlays the target lane from `run-name` (`Deploy Version — stg|tst|prd`) during `verify-promotion-order` before Deployments exist. Stg/Prod Deployments are fetched **per env** (`?environment=stg|prd`, small `per_page`) so busy lower envs cannot hide recent prod.
