...
- **Env** (deploy status / Actions API): **`GITHUB_APP`** first (installation token from `GITHUB_APP_ID` + `GITHUB_APP_INSTALLATION_ID` + `GITHUB_APP_PRIVATE_KEY` — org install on CPT-Group; App ID `4438130`, install `150147363`) → **`GH_MASTER_PAT_KYLE`** → **`GH_ROY_PAT_MASTER_CLASSIC`** → **`GITHUB_TOKEN_3`** → **`GITHUB_TOKEN_2`** → **`GITHUB_DEPLOY_READ_TOKEN`** (any subset may be set; server only; never `NEXT_PUBLIC_*`). Prefer the org GitHub App over personal classic PATs. Tokens with core rate-limit remaining under 50 are skipped (including the last PAT — never burn 0/5000). Classic PATs share the **5000/hr core budget per GitHub user**; once a user is exhausted, other env vars for that same account are skipped (`GH_MASTER_PAT_KYLE` and `GITHUB_DEPLOY_READ_TOKEN` are both Kyle/`XYIAN`; Roy’s classics share `Afro104`). App installation tokens use a separate budget. If a token hard-fails (rate limit, 401, 403, **404**, bad credentials) or Deployments is degraded, the route **advances** to the next **different-user** token. When every user budget is empty, serve stale cache or a clear 503 — do not paint rate-limit text on every card. Tokens need **`actions:read`** on the monitored repos (and **`repo`** if private). **`GET /api/github/deploy-status`** calls the GitHub REST API for monitored workflow IDs in **`GITHUB_DEPLOY_WORKFLOW_MONITORS`** (`src/constants/GITHUB_DEPLOY_MONITORS.ts`). Lane assignment uses **`GITHUB_DEPLOY_LANE_WORKFLOWS.ts`**: standardized repos (AZF, EF migrations, internal-tools) map **Dev** to Dev Fast primary, **Tst** to TST Build primary, **Stg/Prod** to Deploy Version, with active Deploy Version on `development` allowed to surface as In Progress. Local probes: **`npm run test:github-deploy-tokens`**, **`npm run test:deploy-lane-mapping`**, and live **`npm run verify:deploy-lane-mapping -- <repo-slug>`** (uses `gh api` + `gh run list` against the same lane picker as the TV cards; reads `.env.local` for REST fallback).
...
- **Dev Corner One (LEFT TV)** — **Developer-focused**. Closest to the dev desks. Single-view layout:
  ...
  - Middle left: **Work Hours Today** — horizontal bar chart showing hours logged today (Pacific time) in fixed order **Kyle → Roy → James → Brandon** (`NOVA_CORE_DEVS_ACCOUNT_ID_ORDER`). Uses `useWorkHoursToday` hook (10-min poll). Data from Jira worklog API via `/api/jira/worklogs-today`.
  - Middle right: Component Activity table (per-component: open, today, this week).
  - Bottom: NOVA Team Activity panel (`NOVA_CORE_DEVS` cards in the same Kyle → Roy → James → Brandon order, with in-progress + To Do ticket chips). Carlos stays off these panels via `NOVA_CORE_DEVS_EXCLUDED_ACCOUNT_IDS`.
...
- **Dev Corner Two (RIGHT TV)** — **Company-facing**. Visible to non-dev employees. Carousel slides and dwell times are configured in **`devCornerTwoSlides.config.ts`**: each slide has **`enabled: true | false`** and **`durationMs`** (e.g. **25s** for Jira slides, **300s** for GitHub). Only enabled slides are mounted and rotated; order is fixed in that file. **`DEV_CORNER_TWO_FIXED_SLIDE_INDEX`** pins a **0-based index among enabled slides only** (e.g. `0` when GitHub is the sole enabled slide); `null` = auto-advance.
  ...
  - **GitHub — CD deploy status** (`GithubDeployStatusSlide`): **`GET /api/github/deploy-status`** (Actions API; token chain `GITHUB_APP` → `GH_MASTER_PAT_KYLE` → `GH_ROY_PAT_MASTER_CLASSIC` → `GITHUB_TOKEN_3` → `GITHUB_TOKEN_2` → `GITHUB_DEPLOY_READ_TOKEN`).
...
- For Jira **issue** ops (search, comments, transitions), Atlassian MCP is fine; for **Automation** rule enable/disable/config, use repo `scripts/jira/` helpers + `.env.local` (**`JAMES_*`** — needs global Administer; `KYLE_*` returns 403).
- Run routine Jira hygiene via existing skills and promoted scripts (`cpt-jira-stale-ticket-warning`, `cpt-jira-add-component`, `cpt-sr-dev-time-sync`, `enable-move-to-sprint-rule.mjs`, `remediate-move-sprint-orphans.mjs`) rather than ad-hoc tracked one-offs.
- Sync Kyle/James daily hours on **In Dev** tickets via **`/cpt-sr-dev-time-sync`** (`set-in-progress-worklogs-today.mjs`); **always `--apply` immediately** — never dry-run or ask for confirmation; supports absolute hours, **`k:+1` / `j:-0.5` deltas** from today's total, and **21h max/person/day**; **always split evenly across all that person’s In Dev tickets** (never concentrate hours on a single “main” ticket); add **`--include-dev-review`** only when the user asks to include Dev Review; use **`adjust-worklogs-today.mjs`** only to trim/scale existing today entries proportionally by ticket.
...
- Dev Corner One **Team Activity** / **Work Hours**: scoped to **`NOVA_CORE_DEVS`** (not full `NOVA_TEAM`); in-progress chips bucket by **assignee** (not Tech Owner); **To Do** chips render **after** in-progress chips via `--team-activity-todo-*` tokens — differs from Dev Corner Two in-progress cards (Tech Owner).
- Stale UAT warning JQL is project-wide (`status = UAT`, `duedate < startOfDay()`), not `openSprints()` — overdue UAT tickets often leave the active sprint while still in UAT.
- Dev Corner Two **P2P Go** deploy card: **Dev Fast Deploy** for dev; **CD - Promote** for tst/stg/prod — all on `development`; lane assignment uses **`p2pDeployEnvironment.ts`** promotion-order resolution (GitHub Deployments use `onprem-*`, not branch names). P2P **Stg** from Deployments `onprem-nonprod` accepts **Deploy Version only** — Dev Fast also writes that env and must not light Stg.
- Dev Corner Two **CD deploy cards** watch swim-lane workflows only (Dev Fast / TST Build / Deploy Version / in-flight **TST Auto-Merge** on AZF, internal-tools, EF, NuGet, P2P) — **not** CI, PR auto-merge, or Dependabot; a busy Actions/PR page can look active while the TV stays idle. NuGet is **2-lane** Prerelease/Release (legacy Publish NuGet workflow deleted). Poll: **`GITHUB_DEPLOY_STATUS_POLL_INTERVAL_MS`** 25s + ~20s server cache (webhook activity feed still 60s). **N/A** is reserved for package/npm libs that never deploy — never for empty/failed API data (use **API ERR**); real lanes go Idle only after ~7d with no activity — a recent timestamp with Idle usually means a tip/mapping bug (e.g. GitHub **`inactive`** after success), not true Idle. Deployment status skips superseded **`inactive`** tips (use prior success/failure). In-flight **Deploy Version** overlays the target lane from `run-name` (`Deploy Version - stg|tst|prd`) or job-name env suffix when run-name is missing. Card badge + meter use **lane state only**; orphan monitored CD shows muted **Actions busy** (never a conflicting Queued/In Progress badge). Stg/Prod Deployments fetch **per env** (`?environment=stg|prd`, small `per_page`) so busy lower envs cannot bury recent prod. Expected promotion order is **Dev → Tst → Stg → Prod** (never skip); Idle/OK mismatches across lanes can be tip/mapping bugs or real skipped promotions. When diagnosing lane anomalies, read local **`C:\local_dev\ATLAS\cpt-standards`** (`standards/global` deploy/release + github-actions docs) and account for non-standard deploys gracefully while keeping swim-lane workflows as source of truth. Auth: org GitHub App **`GITHUB_APP`** is primary live (`tokenUsed=GITHUB_APP`); classic PATs are fallback only.
...
- Sprint orphan remediation targets NOVA **To Do** + `sprint is EMPTY` (not merely `sprint not in openSprints()` — closed-sprint carryover is intentional).
- Conference room reel copy lives in **`src/constants/CONFERENCE_REEL.ts`** (`CONFERENCE_REEL_PHRASES` for **`TextScroller`** on `/tv/conference-room`).
- **`jose`** npm package is the JWT library for Cursor analytics Entra auth middleware (Edge-safe); it is in `package.json` — run **`npm install`** if `Can't resolve 'jose'` after clone/pull.
- Dev Corner Two deploy-card **footer ticker**: auto-ported themes set `--github-deploy-footer-ticker-text` to `headerBg` (same as bg; GitHub Dark was 1:1 invisible). Footer ticker falls back to **`--text-color`**; audit/repair via **`scripts/fix-github-deploy-ticker-contrast.mjs`**; theme generator fixed in **`generate-dashboard-themes-from-ui.ts`**.
