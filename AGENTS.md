...
- **`cursor-analytics-new-screen/**`** (except **`README.md`** + **`.gitkeep`**) — gitignored; CSV dumps stay local. **`npm run cursor-analytics:regen`** → **`kyleOutput/cursor-analytics-summary.json`** for **`/cursor-analytics`**, including optional **`byDayModelRequests`** from the team rollup **`Models Time Series Data`** column (per-model requests and optional token fields for **CSV est.** monetary mode vs **`includeAdmin=1`** Admin API).
