...
- **Dev Corner Two (RIGHT TV)** — **Company-facing**. Visible to non-dev employees. **5-slide** carousel with **per-slide dwell** (`SLIDE_DURATIONS_MS` in `DevCornerTwoDashboard.tsx`): **25s** for slides 1–4; **120s** (2 min) for slide 5 (GitHub deploy):
  ...
  - Slide 4: **Completions by developer** — custom layout (`CompletedByDevSlide`): per NOVA team member, tickets completed **today** (from `closedTodayIssues`) and **earlier this week** (Mon–Fri window through today, from `resolvedLast14`; excludes today’s keys in the “earlier” list to avoid duplicates). No extra Jira API calls.
  - Slide 5: **GitHub — CD deploy status** (`GithubDeployStatusSlide`): **`GET /api/github/deploy-status`** (Actions API + `GITHUB_DEPLOY_READ_TOKEN`). **`DEV_CORNER_TWO_FIXED_SLIDE_INDEX`** can pin the carousel (e.g. `4` for GitHub in 0-based); set to `null` for normal rotation.
  - **Temporarily out of rotation** (commented JSX in `DevCornerTwoDashboard.tsx`, components retained): **Today** close times by component (`TodayComponentVelocitySlide`); **Developer Load Matrix** (`DevLoadMatrixSlide`). Re-enable by uncommenting the block and restoring imports, `SLIDE_DURATIONS_MS` entries, and slide indices.
