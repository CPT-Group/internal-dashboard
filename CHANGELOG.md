...
- **`/cursor-analytics`**: **No auto-refresh** — data loads on page visit and when the user clicks **Refresh** (TV-style polling removed). **Monetary focus**: KPI **$ / req (est.)**; tables add **Est. $ (cycle · pro-rata)** from Admin current-cycle spend; trend chart **right axis USD** (pro-rata daily $ from CSV + usage-event charges via display heuristic). **Default API** is Team **Admin billing** (`CURSOR_ADMIN_API_KEY`); **Enterprise agent-edits** only with **`CURSOR_ANALYTICS_AGENT_EDITS=1`**.
...
- **`/cursor-analytics` accuracy pass**: `/teams/spend` parsing now supports string `userId` and missing `overallSpendCents`; charged-event pagination default increased to **100 pages** (`CURSOR_ANALYTICS_USAGE_EVENTS_MAX_PAGES`) to reduce truncation warnings while keeping rate-limit risk manageable.
...
- **`/cursor-analytics` daily trend (invoice-shaped)**: chart prefers **current-cycle team invoice total** from `/teams/spend` (sum of `overallSpendCents`) **spread across calendar days** by combined usage + included request counts from `/teams/daily-usage-data` (days on or after `subscriptionCycleStart` in the selected range). **Fallback**: summed `/teams/filtered-usage-events` `chargedCents` (metered line items only — often far below full subscription spend). Event parser accepts string `chargedCents` and coerces 10-digit epoch timestamps to ms.
- **`/cursor-analytics` copy cleanup for cost-first view**: removed “Admin API” phrasing from trend/KPI copy so UI reads as team billing cost and period totals, with trend focused on total team cost over time.
...
- **`/cursor-analytics` tab CSV exports**: each detail tab includes **Export CSV** (PrimeReact `DataTable.exportCSV`); Trend exports the same daily series shown on the chart. New helpers: **`src/utils/csvDownload.ts`**, **`src/utils/cursorAnalyticsTeamTrend.ts`**.
