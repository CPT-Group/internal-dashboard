...
- **Liquid Glass visual refresh (Tizen-safe / pre-2020 CSS)** — translucent frosted surfaces across `.p-card`, `.p-panel`, `.p-dialog`, the home theme switcher, and DataTable rows, driven by new shared tokens in `variables.scss` + every theme file: `--glass-card-bg` (panel fill ~56–82% opaque), `--glass-card-bg-strong` (near-opaque sticky table headers / overlays so auto-scrolling rows don't bleed through), `--glass-border` (bright glass rim), `--glass-sheen` (diagonal `linear-gradient` light wash), `--glass-shadow` (drop shadow + inset top highlight), and `--glass-row-stripe`. Translucency comes purely from `rgba()` + `linear-gradient` + `box-shadow` — **no `color-mix()` and no `backdrop-filter`** (uncertain on old Tizen Chromium), so the effect renders on the TVs.
