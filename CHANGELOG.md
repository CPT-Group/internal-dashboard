...
- **Jira automation — due date 24h rule**: **Ensure Due Date is at Least 24 Hours After Creation** (`019d3181-bf73-73f9-92fa-1ee1a1d8ad52`) now skips both IF branches when **Jennifer Forst** (`62475ee8f4079800705791e0`) is the issue creator (`{{initiator.accountId}}` NOT_EQUALS). She can set sprint due dates under 24h without the rule rewriting them or posting the auto-comment. Helpers: `JENNIFER_FORST_ID` in `_jiraAuto.mjs`, `exempt-jennifer-forst-due-date-rule.mjs`, `lookup-user.mjs`.
