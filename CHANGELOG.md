...
- **Dev Corner Two — GitHub deploy lane mapping**: Standardized repos — **Dev** = `Dev Fast Deploy`; **Tst** = `TST Build Artifact` (+ legacy CD / `Deploy Version` fallback); **Stg/Prod** = legacy main CD + `Deploy Version`. Lane rules in **`GITHUB_DEPLOY_LANE_WORKFLOWS`**. Single-CD repos unchanged. Verify: **`node scripts/verify-deploy-lane-mapping.mjs cpt-ef-postgres-migrations`**.
