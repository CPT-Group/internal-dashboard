...
- **Dev Corner Two — GitHub deploy cards**: **`cpt-infra`** (`Terraform apply` workflow) as a live 5th pipeline card with Dev/Tst/Stg/Prod swim lanes; 6th **Coming soon** placeholder (no GitHub API). Monitor config supports live vs placeholder entries in [`GITHUB_DEPLOY_MONITORS.ts`](src/constants/GITHUB_DEPLOY_MONITORS.ts); **`GithubDeployRepoCards`** 3×2 grid; infra repo tone (`--github-repo-infra-*`). Card body uses **`useAutoScroll`** when swim-lane content overflows on smaller TVs (header stays fixed). All six cards share one **`DeployPipelineCard`** layout (header meta, footer ticker, reserved badge/detail slots, fixed env-row height) so grid cells stay equal size. Per-env **deploy version** label (semver from branch/title when present, else GitHub Actions **#runNumber**, else short SHA) from workflow run fields via **`formatDeployVersionLabel`**.
