...
- **Time-aware data refresh**: Replaced fixed 30-min cache TTL with `getJiraCacheTtl()` that returns 20 min during business hours (6 AM–8 PM Pacific) and 60 min off-hours. All stores updated. Dashboards poll every 60s to check staleness; the TTL determines when a re-fetch triggers.
