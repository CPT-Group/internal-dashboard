...
- **Work Hours threshold borders and flash animation**: Work Hours Today bars now have per-bar themed border colors based on hour thresholds (<4h red, 4–6h yellow, 6–7h green, 7–8h yellow, 8–9h orange, >9h red). Danger-zone bars (<4h or >9h) pulse/flash their border width. Bar fill stays the default theme primary. Data labels now show "h" suffix (e.g. "6.5h"). Applied to both Dev Corner One and Trevor's panels. New `borderColors`, `suffix`, and `flashIndices` fields on `HorizontalBarChartData` type.
