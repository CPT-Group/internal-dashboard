...
- **Dev Corner Two — GitHub deploy lane mapping**: Standardized repos (**AZF**, **EF migrations**, **internal-tools**) now use lane-specific workflow rules from **`GITHUB_DEPLOY_LANE_WORKFLOWS`**: **Dev** primary = `Dev Fast Deploy`, **Tst** primary = `TST Build Artifact`, **Stg/Prod** primary = full deploy workflows, with active `Deploy Version` on `development` allowed to surface as In Progress. `GITHUB_DEPLOY_MONITORS` now fetches the full standardized workflow set (including internal-tools Dev Fast/TST Build/Deploy Version). Added fixture tests and script: **`npm run test:deploy-lane-mapping`**.
