...
- **Dev Corner Two — GitHub deploy cards**: Infra monitor now targets the `PR promotion` workflow; environment lanes resolve from promotion titles (`source → target` / `to`), falling back to branch names only when no target is present. Swim-lane version labels now always show GitHub Actions run numbers (or run id).
...
- **GitHub deploy status accuracy (EF migrations)**: Deploy API now supports **multiple workflow IDs per repo card** and merges runs/counts before lane mapping. `cpt-ef-postgres-migrations` now includes both `CD - Run EF Core Migrations` and `CD - Apply DEV Migrations`, so active DEV runs no longer appear as all-green.
