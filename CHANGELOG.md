...
- **Dev Corner Two — GitHub deploy swim lanes misroute tst/stg/prod to Dev**: Standardized CD repos promote every env from `--ref development`, so branch-only lane matching made active Deploy Version runs look like Dev deploys. Removed Deploy Version from the dev lane `active` workflow list; resolve target env via Deployments API (completed), job-name hints `(tst|stg|prd)` for in-progress Deploy Version, and workflow inference for in-progress TST Build → tst. Card header now picks the highest active lane (prod → dev), shows target env + SHA instead of always `development`, and `verify-deploy-lane-mapping` uses the same resolution path as the TV API.
...
- **NOVA impediment Teams mention support**: Added optional Adaptive Card mention support for impediment webhook alerts in `src/services/jira/impediments.ts`. When `JIRA_IMPEDIMENTS_TEAMS_MENTION_USER_ID` is configured (Microsoft Entra Object ID or UPN), alerts post as an Incoming Webhook Adaptive Card with a real Teams user mention (defaults to Brandon Fay display name). Without that ID, alerts now include a plain-text attention label fallback (`JIRA_IMPEDIMENTS_TEAMS_MENTION_LABEL`, default `@Brandon Fay`). Documented new env keys in `README.md`.
- **Three themes to parity with `cpt-internal-tools`** — **Khaki** (light: olive `#556b2f` + coffee `#6f4e37` on warm cream), **Light Synth** (light: violet `#8b2fd6` + cyan `#0e7490` on pale lavender), and **Flight Deck HUD** (dark: phosphor-green `#22e584` + cyan `#22d3ee` HUD on cockpit black-green, amber warnings). New palette-only theme files `src/styles/themes/{khaki,light-synth,flight-deck-hud}.scss` mirror the existing theme structure with **hand-computed concrete hex/rgba** (the main app derives these with `color-mix()`; that is **not** Tizen-safe, so all values are precomputed here). Wired into `appThemeCycle.ts` (`APP_THEME_CYCLE_ORDER`), the `layout.tsx` theme-init `valid` array, `main.scss` `@use` imports, and `ThemeChangeToast` display-name map.
- **Liquid Glass visual refresh (Tizen-safe / pre-2020 CSS)** — translucent frosted surfaces across `.p-card`, `.p-panel` (incl. headers), `.p-dialog`, the home theme switcher, and DataTable rows, driven by a shared **`src/styles/mixins/_glass.scss`** `glass-tokens()` mixin (single source of truth for the sheen-gradient + layered-shadow *structure*) that every theme `@include`s with its own **precomputed `rgba()`** colors. Emitted tokens: `--glass-card-bg` (panel/card/tile fill, **50–60% opaque**), `--glass-card-bg-strong` (90–94% — **only** scroll-sticky DataTable headers, so auto-scrolling rows never bleed through), `--glass-border` (bright glass rim), `--glass-sheen` (**top-down** `linear-gradient` light wash), `--glass-shadow` (drop shadow + optional colored glow + inset top highlight), `--glass-row-stripe`, and `--ripple-bg`. Translucency comes purely from `rgba()` + `linear-gradient` + `box-shadow` — **no `color-mix()` and no `backdrop-filter`** (uncertain on old Tizen Chromium), so the effect renders on the TVs.
- **Glass made genuinely visible (contrast pass)** — each theme's `--glass-card-bg` now uses a tone **distinctly off the page background** (dark themes: a lighter raised surface; light themes: a cool/whiter pane off the near-white page, since alpha alone is invisible there), paired with a stronger rim border, a real drop shadow, and a top-down sheen so panels read as glass without `backdrop-filter`. Per-theme fills: dark-synth `rgba(42,31,78,.58)`, dark `rgba(35,58,82,.56)`, light `rgba(228,236,247,.55)`, ms-access `rgba(249,251,253,.6)`, khaki `rgba(250,245,233,.58)`, light-synth `rgba(255,255,255,.5)`, flight-deck-hud `rgba(12,47,34,.52)`.

### Fixed

- **Removed a `color-mix()` from the shared style layer** — `.p-ripple-element` in `base.scss` used `color-mix(in srgb, …)` (CSS 2023, unsupported on Samsung Tizen TVs), contradicting the "all `color-mix()` removed in v0.1.60" note. Replaced with a precomputed `--ripple-bg` translucent-primary token set per theme.

### Added

- **Confluence (PT space) — legacy SQL data dictionaries**: High-level **CPT2K16** and **10.0.0.5 (website SQL)** dictionary pages under [Database Migrations](https://cptgroup.atlassian.net/wiki/spaces/PT/pages/99680259/Database+Migrations), scoped to databases/cases from **2025+**. Gitignored regen scripts: `kyleOutput/confluence-legacy-dev/discover-sql-dictionaries.mjs`, `create-sql-dictionary-pages.mjs`. `.cursor/skills/cpt-jira-add-tech-owner/SKILL.md` — backfill empty Tech Owner for tickets assigned to Kyle, Roy, James, or Brandon (Roy/Brandon/James assignee → Roy; Kyle → Kyle; never overwrites existing). Script: `scripts/jira/backfill-tech-owner-by-assignee.mjs`.
- **Cursor skill `/cpt-jira-stale-ticket-warning`**: `.cursor/skills/cpt-jira-stale-ticket-warning/SKILL.md` — post ADF **warning panel** on NOVA **UAT** tickets with `duedate < startOfDay()` (due today is OK); skips if `STALE TICKET WARNING` already posted; @assignee mention. Scripts: `stale-uat-ticket-warning.mjs`, `staleUatWarningAdf.mjs`.
- **Operational board verification**: `npm run test:operational-board-analytics` (fixture tests for in-progress, team activity, impediments) and `npm run verify:operational-board` (live Jira pull + invariant checks mirroring `operationalJiraStore`). **`ImpedimentPanel`** on the middle-right column (replaces Component Activity; panel import left commented for quick restore). Derives **active impediments** from open **`Blocks`** issue links against operational open work (`buildImpedimentAnalytics`, `JIRA_IMPEDIMENT_*` constants, extra link-carrier JQL on `operationalJiraStore`). KPI strip **Impediments** count; blocked-story attribution uses Tech Owner. Discovery: `scripts/jira/discover-impediment-links.mjs`; **`npm run test:impediment-analytics`**.
- **Dev Corner Two — GitHub deploy cards**: **`cpt-infra`** (`PR promotion` workflow) as a live 5th pipeline card with Dev/Tst/Stg/Prod swim lanes; 6th **Coming soon** placeholder (no GitHub API). Monitor config supports live vs placeholder entries in [`GITHUB_DEPLOY_MONITORS.ts`](src/constants/GITHUB_DEPLOY_MONITORS.ts); **`GithubDeployRepoCards`** 3×2 grid; infra repo tone (`--github-repo-infra-*`). Card body uses **`useAutoScroll`** when swim-lane content overflows on smaller TVs (header stays fixed). All six cards share one **`DeployPipelineCard`** layout (header meta, footer ticker, reserved badge/detail slots, fixed env-row height) so grid cells stay equal size. Per-env **deploy version** label uses GitHub Actions **#runNumber** (or run id) via **`formatDeployVersionLabel`**.
...
- **One-off Jira ticket scripts/payloads** (use `kyleJira/` or `scripts/common-scripts/` templates instead): `scripts/jira/create-nova-check-screen-kt.mjs`, `scripts/jira/create-nova-consultant-prep-task.mjs`, `scripts/post-jira-NOVA-848.ps1`, `scripts/jira-NOVA-848-comment.json`, `scripts/jira-NOVA-848-worklog.json`.
...
- **Dev Corner One — Work Hours plaid refinement**: Updated the `>=110%` canvas overlay to read as true plaid (crossing red + green diagonal families with subtle dark woven accents and underlay tint) while preserving legacy Tizen-safe rendering, existing label readability, and the 110% trigger behavior.
- **Dev Corner One — Work Hours plaid motion**: Shifted plaid overlay travel to explicit left-to-right horizontal movement on the x-axis while preserving the existing `>=110%` trigger, label visibility, and Tizen-safe canvas rendering/colors.
...
- **Dev Corner Two — GitHub deploy cards**: Infra monitor now targets `CD - Deploy Infrastructure` so development deployments show up on the dashboard (branch-to-lane mapping remains `development` → Dev).
- **Dev Corner Two — GitHub deploy cards**: NuGet pipeline lanes now collapse to **Non-Prod** + **Prod** (development/test/staging grouped), and the placeholder card label now reads **NPM libs** until the repo exists.
- **Dev Corner Two — GitHub deploy lane mapping**: Standardized repos (**AZF**, **EF migrations**, **internal-tools**) now use lane-specific workflow rules from **`GITHUB_DEPLOY_LANE_WORKFLOWS`**: **Dev** primary = `Dev Fast Deploy`, **Tst** primary = `TST Build Artifact`, **Stg/Prod** primary = `Deploy Version` (removed retired legacy CD workflow IDs). `GITHUB_DEPLOY_MONITORS` fetches the current standardized workflow set. **`verify-deploy-lane-mapping`** and **`test-github-deploy-tokens`** derive workflow IDs from app constants (tsx runner + probe list). Added fixture tests and scripts: **`npm run test:deploy-lane-mapping`**, **`npm run verify:deploy-lane-mapping`**.
...
- **`/cursor-analytics`**: **No auto-refresh** — data loads on page visit and when the user clicks **Refresh** (TV-style polling removed). **Monetary focus**: KPI **$ / req**; tables use Admin billing where available. **Admin API** loads only when **Monetary** is **API** and the client sends **`includeAdmin=1`**; **Enterprise agent-edits** only with **`CURSOR_ANALYTICS_AGENT_EDITS=1`** when billing is loaded.
- **`/cursor-analytics` API-first refactor**: period range is now request-driven (`startDate`/`endDate`), dashboard adds **Sprint / Month / Quarter / All** plus custom date inputs, and cost breakdown tabs now use Admin API usage-event data (`chargedCents`) for **Developers**, **Repos**, **Repo × developer**, **Month × developer**, and **By month (cost)**. Extension-focused tabs were removed. CSV remains historical baseline for usage trend context.
...
- **`/cursor-analytics` repo-cost groundwork**: summary parser now supports dedicated **AI edits by repository** and **repo×developer** CSV exports; when Admin usage events do not include repo metadata, repo tabs can allocate charged cost by AI-edits share from those exports, including **% of repo by developer**.
- **`/cursor-analytics` daily trend**: chart uses **per-day `chargedCents`** from `/teams/filtered-usage-events` (÷ 100) with **full per-day pagination**; CSV request-count scaling applies only when the event load is **complete** (disabled when truncated).
- **`/cursor-analytics` billing env & route**: `CURSOR_ANALYTICS_USAGE_EVENTS_MAX_PAGES` → **`0` skips** `/teams/filtered-usage-events`; **`CURSOR_ANALYTICS_USAGE_EVENTS_MAX_PAGES_PER_DAY`** (default 50_000), **`CURSOR_ANALYTICS_USAGE_EVENTS_REQUEST_DELAY_MS`** (default 3100), **`CURSOR_ANALYTICS_USAGE_EVENTS_MAX_RANGE_DAYS`** (default 120, clips to most recent UTC days). **`GET /api/cursor-analytics`**: `maxDuration` **300s**; response **`warnings`** when clipped. Developers table adds **Charged (range)** (email-matched event sum for selected dates).
...
- **`/cursor-analytics` CSV model estimate mode**: toolbar **Monetary** toggle (persisted in `localStorage`) switches charts/tables from **Team Admin API** billing to **CSV-derived estimates** using summary **`byDayModelRequests`** (ingested from the team rollup **`Models Time Series Data`** column at **`npm run cursor-analytics:regen`**) × public list **$/1M** rates in **`src/constants/cursorModelPricingUsdPer1M.ts`**, with optional **overlap calibration** against usage-event **`chargedCents`** when the event load is complete. **Trend (daily)** can overlay **API (usage events)** as a dashed series. **Money (range)** in CSV mode shows team **estimate + usage + $/req**; **Developers** can show **tabular** activity rows; **Month × developer** can estimate via **month pool × `byMonthDeveloper` share** when tabular data exists; **Repos** / **Repo × developer** allocate the **estimated range total** by AI-edit lines when no event repo split. Helpers: **`src/utils/cursorAnalyticsCsvModelMoneyEstimate.ts`**, **`src/utils/cursorAnalyticsCsvMonthDeveloperShare.ts`**, **`src/utils/cursorAnalyticsBillingGuards.ts`**.
...
- **`/cursor-analytics` Money (range) tab** + **`src/utils/cursorAnalyticsMonetaryJoin.ts`**: per-developer table joining cycle **`/teams/spend`** with **range** charged cents and usage/included request counts (derived $/req columns). **Repos** and **Repo × developer** add a **Basis** column: direct **`/teams/filtered-usage-events`** attribution vs **AI-lines × charged** estimate when repo fields are missing.
...
- **`npm run cursor-analytics:reconcile-day`** (`scripts/cursor-analytics/reconcile-usage-events-day.ts`) — one UTC day, full pagination, prints team `chargedCents` and per-email sums for cross-check against Cursor.com Usage.
...
- **`/cursor-analytics` under-counted daily $**: usage events are fetched **per UTC day** with **pagination to completion** (per-day safety cap), throttled for Cursor’s 20 req/min limit, with **`usageEventRowsReturned`** vs **`usageEventsTotalReported`** reconciliation; disk cache uses policy **v2**. Trend no longer misreports ~$0–$1 team days when the team has heavy usage.
- **`/cursor-analytics`**: Wider **repo/workspace** parsing from `/teams/filtered-usage-events` (nested metadata, GitHub URLs, CPT org paths); **truncation** called out in billing messages and empty repo tabs. **KPI** “Cycle billed” vs **Cycle overall (API)** semantics clarified; Developers table relabeled; **Fast premium reqs** column hidden when all zero. **`npm run test:cursor-usage-event-repo`** exercises repo extraction fixtures.
