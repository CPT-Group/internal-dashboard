...
- **`/cursor-analytics` accuracy pass**: trend now uses a single API source (Admin daily usage + charged events) with no CSV-vs-API dual legend; `/teams/spend` parsing now supports string `userId` and missing `overallSpendCents`; charged-event pagination default increased to **100 pages** (`CURSOR_ANALYTICS_USAGE_EVENTS_MAX_PAGES`) to reduce truncation warnings while keeping rate-limit risk manageable.
...
- **`/cursor-analytics` trend simplification**: daily trend chart now shows a single team-cost series (Admin API charged USD only) and removes the separate usage-based series to avoid mixed-metric confusion.
...
- **Cursor analytics** (`/cursor-analytics`, **`GET /api/cursor-analytics`**): **`byDay`** in CSV summary; Team **Admin billing** client **`src/lib/cursorAdminApi.ts`** + disk cache; **`src/utils/cursorAnalyticsProRataUsd.ts`**, **`src/utils/cursorBillingFormat.ts`**; tabs + USD/pro-rata columns + trend chart. **`.gitignore`**: `cursor-analytics-new-screen/**` except **`README.md`** + **`.gitkeep`**. Regen: **`npm run cursor-analytics:regen`** → **`kyleOutput/cursor-analytics-summary.json`**.
